...
import spokes from "./website/config/spokes.json" with { type: "json" };
import { buildFibonacciUrl } from "./website/scripts/fibonacci-routing.js";
...
test("routes Hub cards to owned destination pages", async () => {
  const hub = await readFile(new URL("./website/index.html", import.meta.url), "utf8");
...
test("ships every declared spoke page with the destination loader", async () => {
  for (const spoke of spokes.spokes) {
    const pageUrl = new URL(`./website/${spoke.entry}`, import.meta.url);
...
test("destination loader reconstructs and renders node children", async () => {
  const loader = await readFile(
    new URL("./website/scripts/destination-page.js", import.meta.url),
    "utf8"
  );
...
test("grounds a bidirectional Fibonacci page graph", async () => {
  const topology = JSON.parse(await readFile(
    new URL("./website/config/page-topology.json", import.meta.url),
    "utf8"
  ));
