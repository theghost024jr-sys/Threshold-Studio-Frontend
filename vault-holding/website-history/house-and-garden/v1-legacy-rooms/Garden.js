...
function lagoonFireflies() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    animateFireflyTrail(firefly);
  }
}

function animateFireflyTrail(firefly) {
  const trail = document.createElement("div");
  trail.className = "firefly-trail";
  firefly.parentElement.appendChild(trail);

  firefly.addEventListener("mousemove", () => {
...
function setupLoreObjects() {
  document.querySelectorAll(".lore-object").forEach((obj) => {
    obj.onclick = () => {
      alert(obj.dataset.text);
    };
  });
}
...
function setupUI() {
  const herb = document.getElementById("nav-herb");
  const greenhouse = document.getElementById("nav-greenhouse");
  const lagoon = document.getElementById("nav-lagoon");
  const cave = document.getElementById("nav-cave");
  const meadow = document.getElementById("nav-meadow");
  const shore = document.getElementById("nav-shore");
  const camp = document.getElementById("nav-camp");
...
function setupUIButtonNav() {
  document.querySelectorAll("#ui-nav button").forEach((btn) => {
    btn.onclick = () => {
      const target = btn.dataset.target;
      const index = rooms.indexOf(target);
      if (index !== -1) showRoom(index);
    };
  });
}
