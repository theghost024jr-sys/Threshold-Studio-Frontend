...
- `vault-holding/website-history/house-and-garden`: Detailed multi-file House & Garden chamber archive.
...
`website/config/fibonacci-routes.json` is the runtime route contract. The design manifests under `vault-holding/website-history/house-and-garden/fib*/` preserve the implementation scaffold without publishing it. Historical snapshots are not assigned to named runtime versions until their provenance and intended experience are both verified.